...
function FillComboBy2ComboWithSel(obj1,obj2,objdesc,arr,exparr,sel){
	for (i = objdesc.length - 1; i>=0; i--) {
		objdesc.removeChild(objdesc.options[i]);
	}
	for(var i=0;i<arr.length;i++){
...
function loopSelected(idCombobox,idListUser)
{
  var txtSelectedValuesObj = document.getElementById(idListUser);
  var selectedArray = new Array();
  var selObj = document.getElementById(idCombobox);
  var i;
  var count = 0;
...
function createTextArea(id,name,initvalue,onchangeaction,style){
	...
	if(!document.all){
		...
		html = "<div id='DIV_"+id+"' class="+style+" contenteditable='true' onkeydown='return EditorKillEnter(this)' onblur='document.getElementById(\""+id+"\").value=this.innerText;"+onchangeaction+"' style='width:98%; border:0;'>"+initvalue.replace(/\n/g,"<br>")+"</div>";
		html += "<textarea style='display:none' name='"+name+"' id='"+id+"' cols='12' rows='4'>"+initvalue+"</textarea>";
	}
	return html;
}

function createInputAutoComplete(id,name,initvalue,onchangeaction,style,data,okaction){
	html = "";
	if(!document.all){
		html = "<textarea id='"+id+"' onfocus='at_Load(this.id,\""+id+'_Hidden_Id'+"\",DATA_"+data+",true,\""+okaction+"\");' autocomplete='off' onkeyup='at_Display(event)' onkeydown='at_KeyDown(event)' onkeypress='fitToText(this)' onclick='cancelEvent(event)' class="+style+" style='height:auto;border:0px solid;width:95%;overflow-y:hidden;overflow-x:hidden' name="+name+" onchange='"+onchangeaction+"'>"+initvalue+"</textarea>\n";
        html += " <input type='hidden' name='' id='"+id+'_Hidden_Id'+"' value='' /> \n";
	}else{
		html = "<div id='DIV_"+id+"' class="+style+" contenteditable='true' onkeydown='return EditorKillEnter(this)' onblur='document.getElementById(\""+id+"\").value=this.innerText;"+onchangeaction+"' style='width:98%; border:0;'>"+initvalue.replace(/\n/g,"<br>")+"</div>";
		html += "<textarea style='display:none' name='"+name+"' id='"+id+"' cols='12' rows='4'>"+initvalue+"</textarea>";
	}
...
function calendarFull(value,id,functionname,action){
	var year = new Date().getFullYear();
	return '<input type="text" size=10 name="'+id+'" id="'+id+'" value="'+value+'" onblur="getFullDate(this);if(typeof('+functionname+')==\'function\')eval(\''+action+'\');"><img src="/images/calendar.png" onclick="document.getElementById(\''+id+'\').focus();var dp_cal_'+id+' = null;					dp_cal_'+id+'  = new Epoch(\'epoch_popup\',\'popup\',document.getElementById(\''+id+'\'),false,'+year+',\'d/m/Y\',\'ChangeDate('+i+')\');					dp_cal_'+id+'.show();HasEvent=true;"></img>';
}

var idInteval = 0;
var scanRequestCount = 0 ; 
function scan(idObject){
    scanRequestCount = 0;    
    //checkScanResult(idObject); ymsgr:sendIM?kdta_09
    // chưa gọi được chương trình scan
    // Kiểm tra lại có load được div file đính kèm hay không, khi load thì có cả file scan và file đính kèm thông thường được không
    //Kiểm tra có clear Interval được không
    
    var soKyHieu = document.frm.SOKYHIEU.value;
    var username = document.frm.username.value;
    var password = document.frm.password.value;
    var year = document.frm.year.value;
    var d = new Date();
    var m = d.getMonth()+1;
    window.location.href="uniscan:"+idObject+"~"+year+"~"+m+"~"+soKyHieu+"~"+username+"~"+password;
    idInteval = self.setInterval(function(){checkScanResult(idObject)},8000);
    return document.frm.SOKYHIEU.value;
    
}

function checkScanResult(idObject){
    if(scanRequestCount < 450){
        var jx = new Jx();
        jx.setParameter('idObject',idObject);
        jx.success("loadDivFileDinhKem");
        jx.request("/hscv/file/checkfilescan","POST");
    }
}
function loadDivFileDinhKem(){
    if(scanRequestCount == 450){
        window.clearInterval(idInteval);
    }else{
        scanRequestCount++;
    }
    
    if(scanresult > 0){
        loadFileDinhKemByIdObject(idObject);
        //scanRequestCount = 450;
    }
}

function loadFileDinhKemByIdObject(idObject){
    var jxLoadDiv = new Jx();
    jxLoadDiv.loadDiv('idFileDinhKem');
    jxLoadDiv.setParameter('idObject',idObject);
    jxLoadDiv.setParameter('is_new','');
    jxLoadDiv.setParameter('type','-1');
    jxLoadDiv.setParameter('is_nogetcontent','1');
    jxLoadDiv.request("/hscv/file","POST");
}