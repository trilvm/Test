...
function Epoch(name,mode,targetelement,multiselect,year,format,action)
{
	this.state = 0;
	this.name = name;
...
{
	var container = document.createElement('div');
	var todaySelected = document.createElement('input');
	todaySelected.setAttribute('type','button');
	todaySelected.setAttribute('value',"Hôm nay");
	todaySelected.setAttribute('title',"Hôm nay");
	todaySelected.owner = this;
	todaySelected.onclick = function() { this.owner.todaySelections(false);};
	container.appendChild(todaySelected);	
...
//-----------------------------------------------------------------------------
Epoch.prototype.todaySelections = function (returnToDefaultMonth)  //PRIVATE: reset the calendar's selection variables to defaults
{
	this.selectedDates = new Array();
	this.rows = new Array(false,false,false,false,false,false,false);
	this.cols = new Array(false,false,false,false,false,false,false);
	if(this.tgt)  //if there is a target element, clear it too
	{
		...
		if(mm<10){mm='0'+mm}
		if(this.calendar.format=='d/m'){
			this.tgt.value = dd+'/'+mm;
		}else{
			this.tgt.value = dd+'/'+mm+'/'+yyyy;
		}
		if(this.mode == 'popup') {//hide the calendar if in popup mode
			this.hide();
		}
...
/*****************************************************************************/
//-----------------------------------------------------------------------------
function CalCell(owner,tableCell,dateObj,row,action)
{
	...
	if(!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
	{
		...
		else //if we can only select one cell at a time
		{
			...
			if(owner.tgt) //if there is a target element to place the value in, do so
			{
				...
				//alert('sss');
				//alert(owner.action);
				if(typeof(ChangeDate)=="function" && HasEvent==true)eval((owner.action==""?"ChangeDate()":owner.action));
			}
		}
