...
function createTextArea(id,name,initvalue,onchangeaction,style){
	html = "";
	if(!document.all){
		html = "<textarea id='"+id+"' class="+style+" onkeypress='fitToText(this)' style='height:auto;border:0px solid;width:98%;overflow-y:hidden;overflow-x:hidden' name="+name+" onchange='"+onchangeaction+"'>"+initvalue+"</textarea>";
	}else{
...
function calendarFull(value,id,functionname,action){
	return '<input type="text" size=10 name="'+id+'" id="'+id+'" value="'+value+'" onblur="getFullDate(this);if(typeof('+functionname+')==\'function\')eval(\''+action+'\');"><img src="/images/calendar.png" onclick="document.getElementById(\''+id+'\').focus();var dp_cal_'+id+' = null;					dp_cal_'+id+'  = new Epoch(\'epoch_popup\',\'popup\',document.getElementById(\''+id+'\'),false,2011,\'d/m/Y\',\'ChangeDate('+i+')\');					dp_cal_'+id+'.show();HasEvent=true;"></img>';
}