...
for unopath in extrapaths:
    if os.path.exists(os.path.join(unopath, "pyuno.so")):
        filename = "pyuno.so"
    elif os.path.exists(os.path.join(unopath, "python26.dll")):
        filename = "python26.dll"
    else:
        continue
...
class DocumentConverter:
    ...
    def convert(self, inputFile, outputFile):
        ...
        try:
            document.storeToURL(outputUrl, self._toProperties(FilterName=filterName,FilterOptions="UTF8,CRLF,,,"))
        finally:
            document.close(True)
