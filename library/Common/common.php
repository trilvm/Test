...
class QLVBDHCommon {
    ...
    static function useDlgConfirm() {
        ...
        $html = '<div id="confirm_dlg" style="position:fixed;" >';
        $html.= '<div>';
    ...
    static function AutoComplete($data, $fieldid, $fieldname, $controlid, $controltext, $fullcompare, $style, $onokaction, $defaultid, $defaultname) {
        ...
        foreach ($data as $item) {
            ...
            $html.="DATA_" . $controlid . "[DATA_" . $controlid . ".length]=new Array('" . str_replace("'", "\\'", $item[$fieldid]) . "','" . str_replace("'", "\\'", $item[$fieldname]) . "');";
        }
        $html.="
				</script>
			";
        $html.="
				<input autocomplete=off onclick='cancelEvent(event)' class=autocombobox value='".htmlspecialchars($defaultname,ENT_QUOTES)."' type=text style='$style' name=$controltext id=$controltext  onkeydown='at_KeyDown(event)' onkeyup='at_Display(event)' onfocus=\"at_Hide(true);at_Load('$controltext','$controlid',DATA_$controlid," . ($fullcompare == true ? "true" : "false") . ",'$onokaction');\">
				<input type=hidden style='$style' name=$controlid id=$controlid value='$defaultid'>
			";
        return $html;
    }
    
    static function showAutoComplete($id,$name,$data){
        $dataHtml = '
            <input id="'.$id.'" 
            class="inputContent"
            type="text" 
            onfocus="at_Load(\''.$id.'\',\''.$name.'\',DATA_'.$data.',true,\'\');" 
            onkeyup="at_Display(event)" 
            onkeydown="at_KeyDown(event)" 
            name="'.$id.'" 
            style="width:200px" 
            value="" 
            ...
            autocomplete="off" 
            title="3">
            <input id="'.$name.'" type="hidden" value="3" name="'.$name.'" style="width:200px">
        ';
        return $dataHtml;
    }
    ...
    //baotq thêm
    static function GetTreeByNameOrder($value, $fieldname, $tree, $tablename, $fieldid, $fieldidparent, $parentvalue, $level,$order) {
        global $db;
        $r = $db->query("SELECT *,$level as LEVEL from $tablename where  $fieldname like ?  and $fieldidparent=? order by $order",array($value,$parentvalue));
        $rows = $r->fetchAll();
        $r->closeCursor();
    ...
    static function buildMenuUL_LI($menuData) {
        ...
        for ($pos = 0; $pos < count($menuData); $pos++) {
			if($menuData[$pos + 1]["LEVEL"] <= $menuData[$pos]["LEVEL"]){
				$action = "href='" . ($menuData[$pos]["URL"] == "" ? $menuData[$pos]["URL_ACTION"]."?a=".time() : $menuData[$pos]["URL"]."?a=".time()) . "'";
			}else{
				$action = "";
			}
            if ($menuData[$pos + 1]["LEVEL"] > $menuData[$pos]["LEVEL"]) {
    ...
    static function calendarFull($value, $name, $id) {
        return '
				<input type="text" size=10 name="' . $name . '" id="' . $id . '" value="' . $value . '" if(typeof(ChangeDate)==\'function\')eval(\'ChangeDate()\');"><img id="I'.$id.'" src="/images/calendar.png" align="absmiddle" onclick="document.getElementById(\'' . $id . '\').focus();var dp_cal_' . $id . ' = null;
    ...
    static function writeSelectDepartmentUserWithSelreport($DName, $UName) {
        ...
        if ($userauth->DEPLEADER == 1 || hosocongviecModel::isVanthuLuutru($userauth->ID_U)==TRUE) {
            $html.=" <option value=0>--Chọn phòng--</option>";
            for ($i = 0; $i < count($department); $i++) {
                $html.="<option value=" . $department[$i]["ID_DEP"] . ">" . str_repeat("--", $department[$i]['LEVEL']) . $department[$i]["NAME"] . "</option>";
            }
    ...
    static function writeSelectDepartmentUserWithSel($DName, $UName, $sel, $depparent) {
        ...
        $arr_user = 'ARR_' . $UName;
        $department = array();
        QLVBDHCommon::GetTree(&$department, "QTHT_DEPARTMENTS", "ID_DEP", "ACTIVE=1 AND ID_DEP_PARENT", $depparent, 1);
    ...
    static function getFullUserByDepId($return, $iddep) {
        ...
        $r = $db->query("
				...
				FROM				
					...
					INNER JOIN QTHT_DEPARTMENTS DEP ON E.ID_DEP=DEP.ID_DEP
				WHERE
					DEP.ID_DEP in ($iddep," . implode(",", $dep) . ") AND U.ACTIVE = '1'
				ORDER BY
					U.ORDERS, E.LASTNAME
			");
        $user = $r->fetchAll();
        ...
        $r = $db->query("
				...
				FROM				
					QTHT_DEPARTMENTS
				WHERE
					ID_DEP_PARENT in ($iddep) AND ACTIVE=1
			");
        $dep = $r->fetchAll();
        foreach ($dep as $item) {
    ...
    static function writeMultiSelectDepartmentUser($DName, $UName, $includesmsemail=false) {
        ...
        $r = $db->query("
				SELECT
					G.ID_G,DEP.ID_DEP,U.ID_U,CONCAT(E.FIRSTNAME , ' ' , E.LASTNAME) as NAME, U.ORDERS, E.LASTNAME,U.USERNAME,E.PHONE,E.EMAIL, u.ACTIVE
				FROM				
					QTHT_USERS U 
					INNER JOIN QTHT_EMPLOYEES E ON E.ID_EMP = U.ID_EMP
					INNER JOIN QTHT_DEPARTMENTS DEP ON E.ID_DEP=DEP.ID_DEP
				...
				SELECT
					G.ID_G,-1 as ID_DEP,U.ID_U,CONCAT(E.FIRSTNAME , ' ' , E.LASTNAME) as NAME, U.ORDERS, E.LASTNAME,U.USERNAME,E.PHONE,E.EMAIL, u.ACTIVE
				FROM				
					QTHT_USERS U 
					INNER JOIN QTHT_EMPLOYEES E ON E.ID_EMP = U.ID_EMP
					INNER JOIN FK_USERS_GROUPS G ON G.ID_U=U.ID_U
				...
				SELECT
					-1 as ID_G,-1 as ID_DEP,U.ID_U,CONCAT(E.FIRSTNAME , ' ' , E.LASTNAME) as NAME, U.ORDERS, E.LASTNAME,U.USERNAME,E.PHONE,E.EMAIL, u.ACTIVE
				FROM				
					QTHT_USERS U 
					INNER JOIN QTHT_EMPLOYEES E ON E.ID_EMP = U.ID_EMP
				WHERE U.ACTIVE = 1
    ...
    static function writeMultiSelectDepartmentUserOnCorporation($DName, $UName, $includesmsemail=false) {
        ...
        $arr_user = 'ARR_' . $UName;
        $department = array();
        QLVBDHCommon::GetTreeWithWhere(&$department, "QTHT_DEPARTMENTS", "ID_DEP", "ACTIVE=1 AND ID_DEP_PARENT", 1, 1, "ID_CQ=" . (int) $user->ID_CQ);
        $r = $db->query("
				SELECT
					ID_G,NAME
				FROM				
	...
	static function MysqlDateToVnDateWithTime($mysqldate) {
        if ($mysqldate != "") {
            return date("d-m-Y H:i",strtotime($mysqldate));
        }
		return "";
    }
	...
	static function countdateswitch($ngay, $arrngaynghilam) {
        $tre = 0;
        $curdate = time();
		$multiple = 1;
        if ($ngay > $curdate) {
	...
	static function trehantostrwithprestring($tre, $ngay, $hanxuly, $prestring) {
        if ($tre > 0) {
            $r = " <font color=red><i>($prestring Trễ " . (floor($tre / 8) > 0 ? floor($tre / 8) . " ngày " : "") . ($tre % 8 > 0 ? ($tre % 8) . " giờ" : "") . ")</i></font>";
        } else if ($tre < 0) {
            /*
              $ngay = strtotime($ngay);
              $ngay = QLVBDHCommon::addDate($ngay,$hanxuly);
             ...
             */
            $tre = $tre * -1;
            $r = " <font color=blue><i>($prestring Còn " . (floor($tre / 8) > 0 ? floor($tre / 8) . " ngày " : "") . ($tre % 8 > 0 ? ($tre % 8) . " giờ" : "") . ")</i></font>";
        }
        return $r;
    }
    ...
    function createTextHanXuLyGiaoViec($value,$type,$date){     
        if($date!=null){
            $hanxuly = (int)$value;
            if((int)$type==8){
                $hanxulycuoi = QLVBDHCommon::addDategio(strtotime($date), $hanxuly);
                $hanxuly = ((int)$hanxuly) /8;
            }else{
                $hanxulycuoi = QLVBDHCommon::addDate(strtotime($date), $hanxuly);
            }          
            if(ceil($hanxuly)>0){
                $html=date('H:i',$hanxulycuoi)." ngày ".date('d/m/Y',$hanxulycuoi)."";
            }else{
                $html= "Không có hạn xử lý.";
            }        
            return (int)$value.' '.((int)$type ==8?'giờ':'ngày').' (Đến '.$html.')';
        }else{
            return (int)$value.' '.((int)$type ==8?'giờ':'ngày');
        }
    }
	...
	//hungph-->selec user 
			...
			$html.="";
			$html.="<select style='width:25%;display:none' name=G$DName id=G$DName onchange='FillComboBy2Combo(this,document.getElementById(\"$DName\"),document.getElementById(\"$UName\"),$arr_user,arr_user_temp);'>";
			$html.="        	<option value=-1>--Chọn nhóm--</option>";
			for($i=0;$i<count($group);$i++){
				$html.="        <option value=".$group[$i]["ID_G"].">".$group[$i]["NAME"]."</option>";
			}
			...
			$html.=" Phòng
			";
			$html.="<select style='width:25%;' name=$DName id=$DName onchange='FillComboBy2Combo(document.getElementById(\"G$DName\"),this,document.getElementById(\"$UName\"),$arr_user,arr_user_temp);'>";
			$html.="        	<option value=-1>--Chọn phòng--</option>";
			for($i=0;$i<count($department);$i++){
				$html.="        <option value=".$department[$i]["ID_DEP"].">".str_repeat("--",$department[$i]['LEVEL']).$department[$i]["NAME"]."</option>";
			}
			...
			$html.=" Ông/Bà
			";
			$html.="<select style='width:25%;' name=$UName id=$UName size=1 ondblclick=\"if(typeof(InsertIntoArr)=='function')eval('InsertIntoArr()');\"></select>";
		...
		static function writeMultiSelectDepartmentUser2($DName,$UName,$includesmsemail=false){
			...
			$html.="
				<table>
					<tr style='display:none'><td style='width:50px;'><b>Nhóm</b></td><td>
			";
			$html.="<select  name=G$DName id=G$DName onchange='FillComboBy2Combo(this,document.getElementById(\"$DName\"),document.getElementById(\"$UName\"),$arr_user,arr_user_temp);'>";
			$html.="        	<option value=-1>--Chọn nhóm--</option>";
			for($i=0;$i<count($group);$i++){
			...
			$html.="        </select>";
			$html.="</td></tr>
					<tr><td style='width:50px;'>Phòng</td><td>
			";
			$html.="<select name=$DName id=$DName onchange='FillComboBy2Combo(document.getElementById(\"G$DName\"),this,document.getElementById(\"$UName\"),$arr_user,arr_user_temp);'>";
			$html.="        	<option value=-1>--Chọn phòng--</option>";
			for($i=0;$i<count($department);$i++){
			...
			$html.="        </select>";
			$html.="</td></tr>
					<tr><td style='width:50px;'>Người</td><td>
			";
			$html.="<select name=$UName id=$UName size=5 multiple ondblclick=\"if(typeof(InsertIntoArr)=='function')eval('InsertIntoArr()');\"></select>";
			$html.='
	...
	static function writeSelectDepartmentWithMultiUser($transition_id, $DName, $UName, $includesmsemail=false) {
        global $db;
    ...
    static function getTreHanNgay($ngaydukien){
        ...
        if($ngay ==0 )
        {
        return '<font style="color:blue;">(Đến hạn)</font>';
        }elseif($ngay > 0)
        {
        return '<font style="color:blue;">(Còn '.$ngay.' ngày)</font>';    
        }elseif($ngay < 0)
        {
        return '<font style="color:red;">(Quá hạn '.abs($ngay).' ngày)</font>';    
        }    
    }
    // 7/8/2018 vuld add func tinh ngay tre 
    static function getTreHanNgayRutGon($ngaydukien){
        $ngaydukien = strtotime($ngaydukien);
        $ngayhientai = $ngayhientai = strtotime(date('d-m-Y'));
        $datediff = $ngaydukien - $ngayhientai;
        $ngay = floor($datediff / (60*60*24));
        ...
        {
            return '<font style="color:blue;">'.$ngay.' ngày</font>';    
        }elseif($ngay < 0)
        {
            return '<font style="color:red;">'.abs($ngay).' ngày</font>';    
        }    
	}  	
    //vuld end     
}
