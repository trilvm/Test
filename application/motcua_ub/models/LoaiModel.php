...
class LoaiModel extends Zend_Db_Table
{
	...
	public function GetAllLoaiHscv()
	{
		$r = $this->getDefaultAdapter()->query("
			SELECT
     			ID_LOAIHSCV,loai_hscv.NAME,MASOQUYTRINH
			FROM
				HSCV_LOAIHOSOCONGVIEC loai_hscv
				INNER JOIN wf_processes wf_p on loai_hscv.MASOQUYTRINH = wf_p.ALIAS
				INNER JOIN wf_classes wf_c on wf_p.ID_C = wf_c.ID_C
			where 
				wf_c.ALIAS = 'MOTCUA'
