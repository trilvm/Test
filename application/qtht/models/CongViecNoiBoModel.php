...
class CongViecNoiBoModel extends Zend_Db_Table {
	
	var $_name = "gscv_dmnoibo";
	...
	/**
     * Get all items in VB_LOAIVANBAN table with pairs : ID_DMNB and NAME
     * @return array
     */
	function GetAllCongViecNoiBos()
	{
        $rs = array();
	...
	static function toComboName(){
		...
		$sql = "select `NAME`,`ID_DMNB` from `gscv_dmnoibo` ORDER BY NAME";
		$query = $dbAdapter->query($sql);
		$re = $query->fetchAll();
		foreach ($re as $row){
			echo "<option id='".$row['ID_DMNB']."' value=".$row['ID_DMNB'].">".$row['NAME']."</option>";
		}
	}
	
	static function getNameById($id){
		$dbAdapter = Zend_Db_Table::getDefaultAdapter();
		$sql = "select `NAME` from `gscv_dmnoibo` 
		where `ID_DMNB` = ? ORDER BY NAME
		";
