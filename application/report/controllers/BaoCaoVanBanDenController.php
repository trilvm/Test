...
require_once 'report/models/BaocaovanbandenreportModel.php';
require_once 'report/models/Ad_XulyvanbandenModel.php';
require_once 'qtht/models/LoaiVanBanModel.php';
class Report_BaoCaoVanBanDenController extends Zend_Controller_Action {
	function init(){
	...
	function reportviewAction(){	
		...
		$this->view->data = BaocaovanbandenreportModel::getReportData($fromdate,$todate,$id_svb,$id_lvbs,$is_lienthong);
		$is_in = $param['is_in'];
		if($is_in){
			global $config;
			...
			$this->renderScript("BaoCaoVanBanDen/reportview_in.phtml");
		}
	...
	function reportviewcvtltAction(){
		...
		$is_in = $param['is_in'];
		$is_lienthong = $param['is_lienthong'];
		$this->view->data = BaocaovanbandenreportModel::getReportDatacvtlt($fromdate,$todate,$id_svb,$id_lvbs,$is_lienthong);
		if($is_in){
			global $config;
			$this->view->config = $config;
			if($fromdate == "")
			...
			if($todate == "")
				$todate = "31/12";
			$this->view->thu="Từ ngày"." ".$fromdate." "."đến ngày"." ".$todate;
			$this->renderScript("BaoCaoVanBanDen/reportviewcvtlt_in.phtml");
		}
