...
require_once 'report/models/XulygiaoviecModel.php';
class Report_XulygiaoviecController extends Zend_Controller_Action{
		function init(){
		}

		function indexAction(){
			$this->view->title = "Báo cáo xử lý giao việc tại đơn vị";
		}
...
//                        var_dump($param);
			...
			$this->view->title="Báo cáo xử lý văn bản giao việc từ ngày $fromdate đến ngày $todate";
			...
			$arr_thongke = XulygiaoviecModel::thongke($type,$todate,$fromdate,$arrparam,$arrayidu,$arraydep);
			$this->view->arr_thongke = $arr_thongke;			
			$count = $arr_thongke["dem"];
			//var_dump($count);exit;
			//XulygiaoviecModel::getCountReportData($type,$todate,$fromdate,$arrparam);
			
			//echo $count; exit;
			$this->view->showPage = QLVBDHCommon::PaginatorWithParentSubmit($count,10,$limit,"frm",$page,"/report/Xulygiaoviec/reportview","reportview");
			$this->view->data = XulygiaoviecModel::getReportData($type,$todate,$fromdate,$arrparam,($page-1)*$limit,$limit,$arrayidu,$arraydep);
			...
			if($is_in){
				...
				$this->renderScript("xulygiaoviec/reportview_in.phtml");
		...
		function reportviewexcelAction() {
			...
			header("Content-Disposition: attachment; filename=baocaogiaoviec.xls;");
			...
			$this->view->sel_trangthai = (int)$param["sel_trangthai"];
			$this->view->op_trangthai = $arrparam["op_trangthai"];            			
			$this->view->title="Báo cáo xử lý giao việc từ ngày $fromdate đến ngày $todate";
			$this->view->xuat=1;
			...
			$count = $arr_thongke["dem"];
			//echo $count; exit;
			$this->view->data = XulygiaoviecModel::getReportData($type,$todate,$fromdate,$arrparam,0,$count,$arrayidu,$arraydep);
			//var_dump($this->view->data);exit;
		}
