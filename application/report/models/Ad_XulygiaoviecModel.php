...
, rs1.`ID_PL`,rs1.ID_PI, HANXULY, DATESEND,rs1.IS_FINISH,rs1.IS_THEODOI, rs1.CAP_GIAOVIEC,rs1.HANXULY_GIAOVIEC,rs1.TIENDO_GIAOVIEC,rs1.LOAICV_GIAOVIEC,vdbi.ID_VBDI as ID_VBDI, 
rs1.IS_TRE as IS_TRE_KT , KQ_DEXUAT
from

( select hscv.`ID_HSCV` , wfl.`ID_U_RECEIVE`,wfl.`ID_U_SEND` , DATESEND,hscv.IS_THEODOI,hscv.cap_giaoviec,
...
(select ID_HSCV , ID_PI , IS_THEODOI ,CAP_GIAOVIEC, HANXULY_GIAOVIEC,TIENDO_GIAOVIEC, LOAICV_GIAOVIEC from `".QLVBDHCommon::Table("hscv_hosocongviec")."` hscv1 ) hscv
...
";
		...
		foreach($re as $item){
			...
			if($item["ID_HSCV"] != "")	{
				$list_vbdi = Ad_XulygiaoviecModel::getListVanbandiByIdHSCV($item["ID_HSCV"]);
